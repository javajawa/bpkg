...
/**
 * Open a directory by name for use in `*at(3)` filesystem calls.
 *
 * This can be used to prevent a range of situations where a move or
 * other inode changing operation during the application's run
 * would cause inconsistent results, by anchoring all calls to a
 * particular directory inode.
 *
 * Symlinks are not followed.
 * The directory is opened with as little capability as possible.
 *
 *  - path: The path to open (either absolute or relative to the cwd).
 *
 * Returns the opened file descriptor for use in other calls.
 */
...
int openat_stat_impl( int ctx, char const * const path, struct estat * const stat, int flags )
{
	...
	if ( fd == -1 )
	{
		...
		switch ( code )
		{
			...
			case ELOOP:
				return openat_stat_impl( ctx, path, stat, flags | O_PATH );
